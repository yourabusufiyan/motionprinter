...
import type {
  ...
  connectedPC,
  computerProfile,
} from './../../src/declarations/LordStore.d';
import type {
  uploadFile,
...
class expressAppClass {
  ...
  static inboxDBPath: string = join(
    this.dir[2],
    expressAppClass.db.data.id.toString() + '.json',
  );
  static inboxDB = JSONFileSyncPreset<Inbox>(
    this.inboxDBPath,
    this.defaultInbox(),
  );
  ...
  static startListening(): void {
    ...
    this.db.data.recentlyConnectedPCs = this.db.data.ConnectedPCs.map((el) =>
      merge(el, { isConnected: false }),
    );
  ...
  static startServer(): boolean {
    let o = false;
    console.log(
      'Starting web server for main computer!',
      this.dbPath,
      this.db.data.computerName,
      app.getAppPath(),
    );
  ...
  static beforeStartSever(): void {
    ...
    this.db.data = {
      ...this.defaultLordData(),
      ...this.db.data,
      ...pick(this.defaultLordData(), ['computerName']),
    };
    this.db.write();
  ...
  static handleServerError(error: any) {
    ...
    const bind =
      typeof this.port === 'string'
        ? `Pipe ${expressAppClass.port}`
        : `Port ${expressAppClass.port}`;
    switch (error.code) {
      case 'EACCES':
      ...
      while (true) {
        ...
        this.addresses.filter(async (ip: string, i: any) => {
          ...
          try {
            ...
            let msg =
              ip == this.ip ? 'You are online.' : `"${data}" is online.`;
            ...
            let profile: AxiosResponse<computerProfile> = await axios.get(
              `${url}/profile`,
            );
            this.db.data.ConnectedPCs = this.db.data.ConnectedPCs.map((pc) =>
              pc.ip === ip ? { ...pc, ...profile.data } : pc,
            );
            this.db.write();
            await sleep(100);
            this.win?.webContents.send('reloadDatabase');

            let printers: AxiosResponse<Array<localPrinter>> = await axios.get(
              `${url}/printers/`,
            );
            this.db.data.ConnectedPCs = this.db.data.ConnectedPCs.map((pc) =>
              pc.ip === ip ? { ...pc, ...{ printers: printers.data } } : pc,
            );
            this.db.write();
            await sleep(100);
            this.win?.webContents.send('reloadDatabase');

            let defaultPrinters: AxiosResponse<localPrinter> = await axios.get(
              `${url}/printers/default`,
            );
            this.db.data.ConnectedPCs = this.db.data.ConnectedPCs.map((pc) =>
              pc.ip === ip
                ? { ...pc, ...{ defaultPrinter: defaultPrinters.data } }
                : pc,
            );
            this.db.write();
            await sleep(100);
            ...
            this.addressesProcessing = this.addressesProcessing.filter(
              (el) => el != ip,
            );
          }
        });
    ...
    setTimeout(async () => {
      while (true) {
        ...
        for (const [ip, data] of Object.entries(this.onlineAddresses)) {
          axios.get(`http://${ip}:9457/api/v1/ping`).catch(async () => {
            console.error(`${data.computerName} is offline.`);
            delete this.onlineAddresses[ip];
            this.win?.webContents.send('notification', {
              msg: `${data.computerName} is offline.`,
            });
            this.db.data.ConnectedPCs = this.db.data.ConnectedPCs.filter(
              (a) => a.ip != ip,
            );
            this.db.write();
            await sleep(100);
  ...
  static routesInit(): void {
    this.router.get('/ping', (req: Request, res: any) =>
      res.send(this.db.data.computerName),
    );
  ...
  static async uploadMethod(req: Request, res: Response, next: NextFunction) {
    ...
    sampleFile.mv(uploadPath, function (err) {
      ...
      if (req.body?.cardMaker == 'true') {
        ...
        if (req.body?.makerID) {
          let card = expressAppClass.db.data.cardMaker.find(
            (el) => el.id === req.body?.makerID,
          );
          ...
          expressAppClass.db.data.cardMaker =
            expressAppClass.db.data.cardMaker.map((el) =>
              el.id === req.body.makerID ? card : el,
            );
          res.json(card);
        } else {
  ...
  static printMethod(req: Request, res: Response): void {
    if (req.body?.filename) {
      let file = find(expressAppClass.db.data.toPrintsCommands, [
        'filename',
        req.body.filename,
      ]);
      let fileIndex = findIndex(expressAppClass.db.data.toPrintsCommands, [
        'filename',
        req.body.filename,
      ]);
      console.log(req.body);
      ...
      if (file) {
        ...
        print(file.destination, options)
          .then(() => {
            file.isPrinted = true;
            res.json({ print: 'successful' });
            console.log(
              `Printed file: ${file.filename} : ${file.originalName}`,
            );
          })
          .catch((err) => {
        ...
        expressAppClass.db.data.toPrintsCommands[fileIndex] = {
          ...file,
          ...{ printOptions: options ? options : null },
        };
        expressAppClass.db.write();
      }
    }
  ...
  static profileMethod(req: Request, res: Response): void {
    ...
    let pickResponse = pick(expressAppClass.db.data, [
      'id',
      'ip',
      'username',
      'computerName',
  ...
  static async connectedPCMethod(req: Request, res: Response) {
    ...
    const numbersToRefresh = query?.numbersToRefresh
      ? +query?.numbersToRefresh
      : 31;
    ...
    if (expressAppClass.db.data?.lastCheckConnectedPC) {
      ...
      if (diff < 2 * 60 * 1000 && query.force != 'yes') {
        // 2 minutes
    ...
    let remainingLastOfflinePCs = uniqBy(
      expressAppClass.db.data.recentlyConnectedPCs
    ...
    expressAppClass.db.data.recentlyConnectedPCs = sortBy(
      uniqBy(concat(computers, remainingLastOfflinePCs), 'ip'),
      ['lastSeen'],
    );
  ...
  static initOnLoad() {
    this.db.data.toPrintsCommands = this.db.data.toPrintsCommands.filter(
      (file) => {
  ...
  */
  ...
  static async oroLoading(req: Request, res: Response) {
    ...
    const oropdf = this.db.data.oropdf?.find(el => el.id === id);

    console.log('Received OROPDF info for processing:', oropdf, req.body);

    if (!oropdf?.id) {
      return res.status(400).json({ error: 'id is required' });
    }
    const oroFolder = await createFolder(join(this.dir[4], id));
    const oroProgressStep = 1 / oropdf.files.length * 80;

    oropdf.options = oropdf.options || {};
    oropdf.options.out_dir = oroFolder;

    this.win?.webContents.send('oroLoading', { id, message: 'Processing PDF', progress: 10 });

    for (const [index, file] of (oropdf.files || []).entries()) {
      ...
      await oroPdf2Image(file).then((file) => {
        ...
        this.win?.webContents.send('oroLoading', { id, message: `Processing PDF ${index + 1} of ${oropdf.files?.length}`, progress: Math.round((index + 1) * oroProgressStep) });
      });

    }

    this.win?.webContents.send('oroLoading', { id, message: 'Compressing PDF', progress: 90 });
